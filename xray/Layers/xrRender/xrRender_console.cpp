...
float ps_r__ssaLOD_A = 48.f;
float ps_r__ssaLOD_B = 32.f;
...
float ps_r1_fog_luminance = 1.f;

// R2-specific
Flags32 ps_r2_ls_flags = { R2FLAG_SUN | R2FLAG_EXP_DONT_TEST_UNSHADOWED | R2FLAG_USE_NVSTENCIL | R2FLAG_EXP_SPLIT_SCENE | R2FLAG_EXP_MT_CALC | R3FLAG_DYN_WET_SURF | R3FLAG_VOLUMETRIC_SMOKE | R3FLAG_GBUFFER_OPT | R2FLAG_TONEMAP };	// r2-only

Flags32 ps_r2_ls_flags_ext = { R2FLAGEXT_SSAO_HALF_DATA };

float ps_r2_df_parallax_h = 0.02f;
float ps_r2_df_parallax_range = 75.f;
float ps_r2_tonemap_middlegray = 1.0f;
float ps_r2_tonemap_adaptation = 5.f;
float ps_r2_tonemap_low_lum = 0.001f;
float ps_r2_tonemap_amount = 0.5f;
float ps_r2_ls_bloom_kernel_g = 3.3f;
float ps_r2_ls_bloom_kernel_b = .7f;
float ps_r2_ls_bloom_speed = 10.f;
float ps_r2_ls_bloom_kernel_scale = 1.0f;
float ps_r2_ls_dsm_kernel = .7f;
float ps_r2_ls_psm_kernel = .7f;
float ps_r2_ls_ssm_kernel = .7f;
float ps_r2_ls_bloom_threshold = .3f;
...
Flags32 ps_r3_ls_flags = { R3FLAG_DYN_WET_SURF | R3FLAG_VOLUMETRIC_SMOKE | R3FLAG_GBUFFER_OPT };
...
float ps_r2_dof_kernel_size = 7.0f;
...
public:
	...
	virtual void Execute(LPCSTR args)	
	{
		...
		{
			case 0:
				strcpy_s(_cfg, "rspec_minimum.ltx");
				break;
			case 1:	
				strcpy_s(_cfg, "rspec_low.ltx");
				break;
			case 2:	
				strcpy_s(_cfg, "rspec_default.ltx");
				break;
			case 3:	
				strcpy_s(_cfg, "rspec_high.ltx");	
				break;
			case 4:	
				strcpy_s(_cfg, "rspec_extreme.ltx");
				break;
		}
...
void xrRender_initconsole()
{
    ...
    CMD4(CCC_Float, "r__ssa_lod_b", &ps_r__ssaLOD_B, 32, 64);
	...
	CMD3(CCC_Mask, "r3_msaa_hybrid", &ps_r3_ls_flags, R3FLAG_MSAA_HYBRID);
	CMD3(CCC_Mask, "r3_msaa_opt", &ps_r3_ls_flags, R3FLAG_MSAA_OPT);
	CMD3(CCC_Mask, "r3_gbuffer_opt", &ps_r3_ls_flags, R3FLAG_GBUFFER_OPT);
	CMD3(CCC_Mask, "r3_use_dx10_1", &ps_r3_ls_flags, (u32)R3FLAG_USE_DX10_1);
	CMD3(CCC_Token, "r3_msaa_alphatest", &ps_r3_msaa_atest, qmsaa__atest_token);
	CMD3(CCC_Token, "r3_minmax_sm", &ps_r3_minmax_sm, qminmax_sm_token);

	CMD3(CCC_Mask, "r3_volumetric_smoke", &ps_r3_ls_flags, R3FLAG_VOLUMETRIC_SMOKE);
	...
	CMD3(CCC_Mask, "r3_dynamic_wet_surfaces", &ps_r3_ls_flags, R3FLAG_DYN_WET_SURF);
