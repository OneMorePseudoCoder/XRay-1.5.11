...
void safe_verify(LPCSTR file_name, mp_anticheat::configs_verifyer& verifyer, u8* data, u32 const data_size, string256 & dst_reason)
{
	__try
	{
...
void check_file(LPCSTR file_name)
{
	...
	if (!tmp_reader)
	{
		FS.rescan_physical_path("$screenshots$", TRUE);
