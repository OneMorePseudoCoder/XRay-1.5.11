...
BOOL CTorch::net_Spawn(CSE_Abstract* DC) 
{
	...
	bool dyn_lights_allowed = !!psDeviceFlags.test(rsR2);
	dyn_lights_allowed |= !!psDeviceFlags.test(rsR3);
	...
	m_color = pUserData->r_fcolor("torch_definition", dyn_lights_allowed ? "color_r2" : "color");
	fBrightness = m_color.intensity();
	float range = pUserData->r_float("torch_definition", dyn_lights_allowed ? "range_r2" : "range");
	light_render->set_color(m_color);
	light_render->set_range(range);

	Fcolor clr_o = pUserData->r_fcolor("torch_definition", dyn_lights_allowed ? "omni_color_r2" : "omni_color");
	float range_o = pUserData->r_float("torch_definition", dyn_lights_allowed ? "omni_range_r2" : "omni_range");
